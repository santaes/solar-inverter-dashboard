...
_AUTHORITATIVE_ADDITIONS = (
    (65, "02 Быстрые данные", "Версия ПО инвертора", "только чтение", "uint16_t", 1.0, "", False),
    (66, "02 Быстрые данные", "Статус подключения к BMS", "только чтение", "uint16_t", 1.0, "enum", False),
    (67, "02 Быстрые данные", "Режим работы инвертора", "только чтение", "uint16_t", 1.0, "enum", False),
    (68, "02 Быстрые данные", "Состояние силовых клемм", "только чтение", "uint16_t", 1.0, "bitfield", False),
    (69, "02 Быстрые данные", "Потоки энергии (Power Flow)", "только чтение", "uint16_t", 1.0, "bitfield", False),
    (70, "02 Быстрые данные", "Статус топологии / single", "только чтение", "uint16_t", 1.0, "enum", False),
    (141, "07 Батарея", "BMS: Напряжение точки CV", "только чтение", "uint16_t", 0.1, "V", False),
    (142, "07 Батарея", "BMS: Номинальная емкость", "только чтение", "uint16_t", 0.1, "Ah", False),
    (143, "07 Батарея", "BMS: Текущая емкость", "только чтение", "uint16_t", 0.1, "Ah", False),
    ...
    (400, "07 Батарея", "Выбранный протокол BMS", "только чтение", "uint16_t", 1.0, "", False),
    (403, "07 Батарея", "Статус подключения к BMS (v2)", "только чтение", "uint16_t", 1.0, "enum", False),
    ...
    (800, "08 Температура", "Скорость вращения вентилятора", "только чтение", "uint8_t", 1.0, "%", False),
    (801, "08 Температура", "Статус вентилятора", "только чтение", "uint8_t", 1.0, "", False),
    (816, "08 Температура", "Температура трекера MPPT 1", "только чтение", "uint16_t", 0.1, "°C", False),
    (818, "08 Температура", "Температура зарядного блока (CHG)", "только чтение", "uint16_t", 0.1, "°C", False),
    (819, "08 Температура", "Температура зарядного блока 2 (CHG2)", "только чтение", "uint16_t", 0.1, "°C", False),
    (820, "08 Температура", "Температура внутри корпуса (Ambient)", "только чтение", "uint16_t", 0.1, "°C", False),
    (821, "08 Температура", "Температура цепей разряда (DIS_CHG)", "только чтение", "uint16_t", 0.1, "°C", False),
    (16640, "09 Настройки", "Выходное напряжение (OPV)", "чтение/запись", "uint16_t", 1.0, "V", False),
    (16641, "09 Настройки", "Выходная частота (OPF)", "чтение/запись", "uint16_t", 1.0, "Hz", False),
    (16642, "09 Настройки", "Приоритет питания нагрузки (OPP)", "чтение/запись", "uint16_t", 1.0, "", False),
    (16643, "09 Настройки", "Режим работы инвертора (MOD)", "чтение/запись", "uint16_t", 1.0, "", False),
    (16644, "09 Настройки", "Приоритет заряда АКБ (CHP)", "чтение/запись", "uint16_t", 1.0, "", False),
    (16645, "09 Настройки", "Ток заряда от сети (RCC)", "чтение/запись", "uint16_t", 1.0, "A", False),
    (16646, "09 Настройки", "Макс. общий ток заряда (MCC)", "чтение/запись", "uint16_t", 1.0, "", False),
    (16647, "09 Настройки", "Тип аккумулятора (BAT)", "чтение/запись", "uint16_t", 1.0, "", False),
    (16648, "09 Настройки", "Тревога низкого АКБ (BAL)", "чтение/запись", "uint16_t", 0.1, "V", False),
    (16649, "09 Настройки", "Отключение по низкому АКБ (BAU)", "чтение/запись", "uint16_t", 0.1, "V", False),
    (16650, "09 Настройки", "Напряжение насыщения / Bulk (BCV)", "чтение/запись", "uint16_t", 0.1, "V", False),
    (16651, "09 Настройки", "Напряжение буфера / Float (BFL)", "чтение/запись", "uint16_t", 0.1, "V", False),
    (16652, "09 Настройки", "Напряжение перехода на сеть (BTG)", "чтение/запись", "uint16_t", 0.1, "V", False),
    (16653, "09 Настройки", "Напряжение возврата на АКБ (BTB)", "чтение/запись", "uint16_t", 0.1, "V", False),
    (16654, "09 Настройки", "Нижний порог напряжения сети (LLV)", "чтение/запись", "uint16_t", 1.0, "V", False),
    (16655, "09 Настройки", "Верхний порог напряжения сети (LHV)", "чтение/запись", "uint16_t", 1.0, "V", False),
    (16658, "09 Настройки", "Режим выравнивания заряда (EQM)", "чтение/запись", "uint16_t", 1.0, "", False),
    (16659, "09 Настройки", "Напряжение выравнивания (EQV)", "чтение/запись", "uint16_t", 0.1, "V", False),
    (16660, "09 Настройки", "Время выравнивания (EQT)", "чтение/запись", "uint16_t", 1.0, "min", False),
    (16661, "09 Настройки", "Задержка выравнивания (EQO)", "чтение/запись", "uint16_t", 1.0, "min", False),
    (16662, "09 Настройки", "Интервал выравнивания (EQI)", "чтение/запись", "uint16_t", 1.0, "days", False),
    (16663, "09 Настройки", "Немедленный старт выравнивания (EQN)", "чтение/запись", "uint16_t", 1.0, "", False),
    (16664, "09 Настройки", "Напряжение откл. 2-го выхода (DBC)", "чтение/запись", "uint16_t", 0.1, "V", False),
    (16665, "09 Настройки", "Таймер 2-го выхода (DBT)", "чтение/запись", "uint16_t", 1.0, "min", False),
    (16666, "09 Настройки", "Разрешение работы генератора (GEE)", "чтение/запись", "uint16_t", 1.0, "", False),
    (16667, "09 Настройки", "Номинальная мощность генератора (GEP)", "чтение/запись", "uint16_t", 0.1, "kW", False),
    (16668, "09 Настройки", "Максимальная мощность генератора (GEM)", "чтение/запись", "uint16_t", 0.1, "kW", False),
    (16670, "09 Настройки", "Выбор протокола обмена BMS (BMS)", "чтение/запись", "uint16_t", 1.0, "", False),
    (16671, "09 Настройки", "ID батареи в BMS (BMI)", "чтение/запись", "uint16_t", 1.0, "", False),
    (16672, "09 Настройки", "Отключение по низкому SOC (BSU)", "чтение/запись", "uint16_t", 1.0, "%", False),
    (16673, "09 Настройки", "Возврат на питание от АКБ по SOC (STB)", "чтение/запись", "uint16_t", 1.0, "%", False),
    (16674, "09 Настройки", "Переход на питание от сети по SOC (STG)", "чтение/запись", "uint16_t", 1.0, "%", False),
    (16675, "09 Настройки", "Дистанционное управление питанием", "чтение/запись", "uint16_t", 1.0, "", False),
    (16677, "09 Настройки", "Автогашение экрана LCD (ATB)", "чтение/запись", "uint16_t", 1.0, "", False),
    (16679, "09 Настройки", "Автоперезапуск при перегреве (TrS)", "чтение/запись", "uint16_t", 1.0, "", False),
    (16680, "09 Настройки", "Автоперезапуск при перегрузке (LrS)", "чтение/запись", "uint16_t", 1.0, "", False),
    (16681, "09 Настройки", "Переход на байпас при перегрузке (OLG)", "чтение/запись", "uint16_t", 1.0, "", False),
    (16682, "09 Настройки", "Режим энергосбережения / ECO (PWS)", "чтение/запись", "uint16_t", 1.0, "", False),
    (16686, "09 Настройки", "Экспорт энергии в сеть / Grid-Tie (GTI)", "чтение/запись", "uint16_t", 1.0, "", False),
    (16687, "09 Настройки", "Лимит мощности отдачи в сеть (GTP)", "чтение/запись", "uint16_t", 0.1, "kW", False),
    (16688, "09 Настройки", "Отключение звукового зуммера (MUE)", "чтение/запись", "uint16_t", 1.0, "", False),
    (16691, "09 Настройки", "Сброс на заводские настройки (STD)", "чтение/запись", "uint16_t", 1.0, "", False),
    (16692, "09 Настройки", "Часы RTC: Секунды (SEC)", "чтение/запись", "uint16_t", 1.0, "s", False),
    (16693, "09 Настройки", "Часы RTC: Минуты (MIU)", "чтение/запись", "uint16_t", 1.0, "min", False),
    (16694, "09 Настройки", "Часы RTC: Часы (HOR)", "чтение/запись", "uint16_t", 1.0, "h", False),
    (16695, "09 Настройки", "Часы RTC: День (DAY)", "чтение/запись", "uint16_t", 1.0, "day", False),
    (16696, "09 Настройки", "Часы RTC: Месяц (MON)", "чтение/запись", "uint16_t", 1.0, "month", False),
    (16697, "09 Настройки", "Часы RTC: Год (YEA)", "чтение/запись", "uint16_t", 1.0, "year", False),
    (16706, "09 Настройки", "Максимальный ток разряда АКБ (MDC)", "чтение/запись", "uint16_t", 1.0, "A", False),
    (16715, "09 Настройки", "SOC отключения 2-го выхода (DBS)", "чтение/запись", "uint16_t", 1.0, "%", False),
    (16719, "09 Настройки", "Конфигурация входов PV", "чтение/запись", "uint16_t", 1.0, "", False),
    (16756, "09 Настройки", "Приоритет заряда: Старт Часы Т1", "чтение/запись", "uint16_t", 1.0, "h", False),
    (16757, "09 Настройки", "Приоритет заряда: Старт Минуты Т1", "чтение/запись", "uint16_t", 1.0, "min", False),
    (16758, "09 Настройки", "Приоритет заряда: Конец Часы Т1", "чтение/запись", "uint16_t", 1.0, "h", False),
    (16759, "09 Настройки", "Приоритет заряда: Конец Минуты Т1", "чтение/запись", "uint16_t", 1.0, "min", False),
)
